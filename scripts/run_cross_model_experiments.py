...
def extract_all_vectors(model_name, traits):
    ...
    act_dir = f"results/activations/{model_short}"
    out_dir = f"results/persona_vectors/{model_short}"
...
def evaluate_steering(model_name, trait="openness"):
    ...
    return run_cmd(
        [
            python,
            "src/evaluation/evaluate_steering_final.py",
            "--model",
            model_name,
            "--trait",
            trait,
...
def generate_cross_model_orthogonality(
    model_configs, output_dir="results/cross_model_results"
):
    ...
    for idx, (model_name, layer) in enumerate(model_configs):
        ...
        if len(found_traits) < 2:
            print(
                f"  Warning: Only {len(found_traits)} vectors found for {model_name} at layer {layer}"
            )
            axes[idx].text(
                0.5,
                0.5,
        ...
        ax.set_title(
            f"{model_label}\n(Layer {layer}, |cos|={np.mean(np.abs(off_diag)):.3f})",
            fontsize=10,
        )
...
def generate_cross_model_layer_profiles(
    model_configs, output_dir="results/cross_model_results"
):
    ...
    for trait in BIG_FIVE[:3]:  # openness, conscientiousness, extraversion
        ...
        for idx, (model_name, _) in enumerate(model_configs):
            model_short = model_name.replace("/", "_")
            analysis_path = f"results/persona_vectors/{model_short}/{trait}/analysis_v2_{trait}.json"
...
def main():
    ...
    parser.add_argument(
        "--traits", type=str, default="all", help="'big5', 'defense', or 'all'"
    )
