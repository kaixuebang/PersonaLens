...
def evaluate_stability(trait_dir, trait_name, output_dir, n_splits=5):
    ...
    if n_samples < 4:
        print(
            f"Not enough samples ({n_samples}) for {trait_name} to do split evaluation."
        )
        return
    ...
    for layer in layers:
        ...
        results["mean_diff_cosine"][layer] = {
            "mean": np.mean(md_sims),
            "std": np.std(md_sims),
        }
        results["probe_cosine"][layer] = {
            "mean": np.mean(pr_sims),
            "std": np.std(pr_sims),
        }
        results["cross_method_cosine"][layer] = {
            "mean": np.mean(cross_sims),
            "std": np.std(cross_sims),
        }
    ...
    ax.plot(
        layers_arr, md_means, "o-", label="Mean-Diff (Set A vs Set B)", color="#2196F3"
    )
    ax.plot(
        layers_arr,
        pr_means,
        "s-",
        label="Linear Probe (Set A vs Set B)",
        color="#FF9800",
    )
    ax.plot(
        layers_arr,
    ...
    ax.set_title(
        f"Vector Stability Across Disjoint Scenarios — {trait_name}", fontsize=13
    )
    ax.legend()
    ...
    best_layer = max(
        intermediate_layers, key=lambda l: results["mean_diff_cosine"][l]["mean"]
    )
    best_cosine = results["mean_diff_cosine"][best_layer]["mean"]

    print(
        f"  ✓ Best intermediate layer (L={best_layer}): MD alignment = {best_cosine:.3f}"
    )
    ...
    results["summary"] = {
        ...
        "intermediate_layers_range": [
            int(intermediate_layers[0]),
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="results/ood_results")
