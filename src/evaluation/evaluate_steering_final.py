    ...
    def evaluate_alpha_sweep(self, trait_name, alphas, eval_prompts):
        ...
        for alpha in alphas:
            ...
            for prompt in eval_prompts:
                ...
                responses.append({"prompt": prompt, "response": resp, "score": score, "perplexity": perp})
            ...
            print(f"  α={alpha:+6.1f}: score={np.mean(scores):+.2f} ± {np.std(scores):.2f} | PPL={np.mean(perplexities):.2f}")
        return results
...
def main():
    ...
    ax1.errorbar(sweep_alphas, means, yerr=stds, fmt='o-', color=color1, linewidth=2, capsize=5)
    ...
    plt.title(f"Steering Trade-off Curve: {args.trait.capitalize()} (Layer {layer})\nShowing inverted-U saturation and perplexity degradation", fontsize=11)
    fig.tight_layout()
    plt.savefig(f"{out_dir}/tradeoff_curve_{args.trait}.png", dpi=150)
    
    # Save a copy to the paper folder
    import shutil
    shutil.copy(f"{out_dir}/tradeoff_curve_{args.trait}.png", "paper/fig_tradeoff_curve.png")
