...
def analyze_trait_v2(trait_dir, trait_name, output_dir, regularization_C=0.01):
    ...
    print(f"  Regularization C: {regularization_C}, CV: LOO" if n_samples*2<=40 else f"  Regularization C: {regularization_C}, CV: StratifiedKFold")
    ...
    best_layer_snr = layer_indices[np.argmax(
    ...
    axes[0,0].plot(layer_indices, probe_accs, "o-", color="#2196F3", linewidth=2, markersize=4, label="Standard CV")
    axes[0,0].plot(layer_indices, loso_accs, "s-", color="#FF5722", linewidth=2, markersize=4, label="LOSO CV")
    ...
    ax2 = axes[0,1]
    ax2.plot(layer_indices, raw_norms, "o-", color="#FF5722", linewidth=2, markersize=4, label="Raw diff norm")
    ax2.fill_between(layer_indices,
                     [m-2*s for m,s in zip(random_baseline_norms, random_baseline_stds)],
                     [m+2*s for m,s in zip(random_baseline_norms, random_baseline_stds)],
                     alpha=0.2, color="gray", label="Random ±2σ")
    ax2.plot(layer_indices, random_baseline_norms, "--", color="gray", linewidth=1, label="Random baseline")
    ...
    axes[0,2].plot(layer_indices, normalized_norms, "^-", color="#4CAF50", linewidth=2, markersize=4)
    ...
    axes[1,0].axhline(y=0.8, color="red", linestyle="--", alpha=0.5, label="Large effect (d=0.8)")
    ...
    axes[1,1].plot(layer_indices, pca_var1s, "^-", color="#FF9800", linewidth=2, markersize=4)
    ...
    axes[1,2].plot(layer_indices, brier_scores, "D-", color="#607D8B", linewidth=2, markersize=4)
    ...
    plt.tight_layout()
    fig_path = os.path.join(output_dir, trait_name, f"layer_analysis_v2_{trait_name}.png")
...
def plot_cross_trait_v2(all_results, output_dir):
    ...
    colors = {"openness": "#2196F3", "conscientiousness": "#FF5722", "extraversion": "#4CAF50",
              "agreeableness": "#9C27B0", "neuroticism": "#FF9800",
              "humor": "#E91E63", "projection": "#00BCD4", "rationalization": "#795548"}
    ...
    for trait_name, results in sorted(all_results.items()):
        ...
        data = results["layers"]
        loso_accs = [data.get(l, data.get(str(l), {})).get("loso_accuracy", 0.5) for l in layers]
        norm_diffs = [data.get(l, data.get(str(l), {})).get("rms_normalized_diff_norm", 0) for l in layers]
        ...
        axes[0].plot(layers, loso_accs, ls, color=color, linewidth=1.5, label=trait_name, alpha=0.8)
        axes[1].plot(layers, norm_diffs, ls, color=color, linewidth=1.5, label=trait_name, alpha=0.8)
        axes[2].plot(layers, ds, ls, color=color, linewidth=1.5, label=trait_name, alpha=0.8)
    ...
    axes[1].set_xlabel("Layer"); axes[1].set_ylabel("Norm / RMS")
    ...
    axes[2].set_xlabel("Layer"); axes[2].set_ylabel("Cohen's d")
...
def main():
    parser = argparse.ArgumentParser(description="Enhanced persona vector extraction (v2)")
    parser.add_argument("--activations_dir", type=str, required=True)
    parser.add_argument("--output_dir", type=str, default=None)
    parser.add_argument("--trait", type=str, default="all")
    parser.add_argument("--regularization_C", type=float, default=0.01,
    ...
    for trait_name in sorted(traits):
        ...
        if not os.path.isdir(trait_dir):
            continue
        results = analyze_trait_v2(trait_dir, trait_name, args.output_dir, args.regularization_C)
    ...
    for t, r in sorted(all_results.items()):
        print(f"  {t:25s} LOSO-best: L{r['best_layer_loso']:3d} "
