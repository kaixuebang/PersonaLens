...
class RefinedPatcher:
    ...
    def _find_token_spans(self, pos_messages, neg_messages):
        ...
        pos_text = apply_chat_template_safe(
            self.tokenizer, pos_messages, tokenize=False, add_generation_prompt=True
        )
        neg_text = apply_chat_template_safe(
            self.tokenizer, neg_messages, tokenize=False, add_generation_prompt=True
        )
        ...
        sys_only_pos = apply_chat_template_safe(
            self.tokenizer,
            [pos_messages[0]],
            tokenize=False,
    ...
    def compute_refined_importance(self, pos_msgs, neg_msgs):
        ...
        for target_layer in range(n_layers):
            ...
            if target_layer in neg_cache["hidden"]:
                ...
                self._clear_hooks()
                results["full_layer"][target_layer] = self._kl_div(
            ...
            if target_layer in neg_cache["hidden"]:
                ...
                hook = layers[target_layer].register_forward_hook(
                    make_span_patch(
                        target_layer, neg_cache["hidden"][target_layer], sys_s, sys_e
                    )
                )
                self.hooks = [hook]
                patched_logits = self._get_logits(pos_inputs)
                self._clear_hooks()
                results["system_tokens"][target_layer] = self._kl_div(
                    clean_logits, patched_logits
                )
            ...
            if target_layer in neg_cache["hidden"]:
                ...
                hook = layers[target_layer].register_forward_hook(
                    make_span_patch(
                        target_layer, neg_cache["hidden"][target_layer], usr_s, usr_e
                    )
                )
                self.hooks = [hook]
                patched_logits = self._get_logits(pos_inputs)
                self._clear_hooks()
                results["user_tokens"][target_layer] = self._kl_div(
                    clean_logits, patched_logits
                )
            ...
            if target_layer in neg_cache["hidden"]:
                ...
                total_len = min(
                    pos_inputs["input_ids"].shape[1],
                    neg_cache["hidden"][target_layer].shape[1],
                )
                ...
                hook = layers[target_layer].register_forward_hook(
                    make_span_patch(
                        target_layer, neg_cache["hidden"][target_layer], rand_s, rand_e
                    )
                )
                self.hooks = [hook]
                patched_logits = self._get_logits(pos_inputs)
                self._clear_hooks()
                results["random_tokens"][target_layer] = self._kl_div(
                    clean_logits, patched_logits
                )

            # --- 4. Attention output patching ---
            if target_layer in neg_cache["attn"] and hasattr(
                layers[target_layer], "self_attn"
            ):
                ...
                patched_logits = self._get_logits(pos_inputs)
                self._clear_hooks()
                results["attn_component"][target_layer] = self._kl_div(
                    clean_logits, patched_logits
                )

            # --- 5. MLP output patching ---
            if target_layer in neg_cache["mlp"] and hasattr(
                layers[target_layer], "mlp"
            ):
                ...
                patched_logits = self._get_logits(pos_inputs)
                self._clear_hooks()
                results["mlp_component"][target_layer] = self._kl_div(
                    clean_logits, patched_logits
                )
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="results/localization")
    ...
    for trait_name in traits:
        ...
        summary = run_refined_localization(
            model, tokenizer, trait_name, args.device, args.n_samples
        )
        ...
        colors = {
            "full_layer": "#2196F3",
            "system_tokens": "#FF5722",
            "user_tokens": "#4CAF50",
            "random_tokens": "#795548",
            "attn_component": "#9C27B0",
            "mlp_component": "#FF9800",
        }
        ...
        for s, c in colors.items():
            ...
            axes[0].plot(
                layers, mean, "-", color=c, linewidth=1.5, label=s.replace("_", " ")
            )
        ...
        axes[1].bar(
            layers, mlp_mean, bottom=attn_mean, color="#FF9800", alpha=0.7, label="MLP"
        )
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(output_dir, f"refined_{trait_name}.png"),
            dpi=150,
            bbox_inches="tight",
        )
        plt.close()
